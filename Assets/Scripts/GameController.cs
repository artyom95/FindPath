...
public class GameController : MonoBehaviour
{
    ...
    [SerializeField] private RecoloringMousePathBehaviour _recoloringMousePathBehaviour;

    [SerializeField] private ChoosePlaceToFinish _choosePlaceToFinish;
    [SerializeField] private ShortestPathProvider _shortestPathProvider;
    ...
    void Update()
    {
        if (_mapBuilder.HasMapBuilded())
        {
            _buttonController.DeactivateTileButtons();
        }
        if (_mapBuilder.HasMapBuilded() && _playerProvider.HasPrefabInstantiated() && !_isPrefabMove)
        {
            ...
            if (Input.GetMouseButtonDown(0) && _recoloringMousePathBehaviour.GetStatusHighlightObject())
            {
                ...
                if (_path.Count > 1)
                {
                    ...
                    _isPrefabMove = true;
                    _playerController.Moving(_playerProvider.Player);
